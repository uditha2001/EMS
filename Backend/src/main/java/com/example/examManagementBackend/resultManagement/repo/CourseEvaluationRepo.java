...
public interface CourseEvaluationRepo extends JpaRepository<CourseEvaluationsEntity,Long> {
    ...
    @Query("SELECT count(ce) FROM CourseEvaluationsEntity ce WHERE ce.courses.code=:courseCode AND ce.examTypes.examType=:type")
    public Integer countByCourseCodeAndExamType(@Param("courseCode") String courseCode, @Param("type") String type);
    ...
    @Query("UPDATE CourseEvaluationsEntity ce " +
            ...
            "WHERE ce.courses.code = :courseCode AND ce.examTypes.examType = :type")
    void updateByCourseCodeAndExamType(@Param("newPassMark") float newPassMark,
                                       @Param("newWeightage") float newWeightage,
                                       @Param("courseCode") String courseCode,
                                       @Param("type") String type);
    @Query("SELECT ce.passMark FROM CourseEvaluationsEntity ce WHERE ce.courses.code=:courseCode AND ce.examTypes.examType=:name ")
    float getPassMarkByCourseCodeAndCourseEvaluationId(String courseCode, String name);
    @Query("SELECT ce.weightage FROM CourseEvaluationsEntity ce WHERE ce.courses.code=:courseCode AND ce.examTypes.examType=:name ")
    float getWeightageByCourseCodeAndCourseEvaluationId(String courseCode, String name);
