...
const PaperPreview: React.FC<PaperPreviewProps> = ({
  ...
  const downloadPaperPDF = () => {
    ...
    doc.text(`${semester} Examination - ${examYear}`, pageWidth / 2, currentY, { align: 'center' });
    currentY += 10;
    ...
    const instructionsLines = doc.splitTextToSize(stripHtml(instructions) || 'No instructions provided.', maxWidth);
        ...
        if (q.subquestions && q.subquestions.length > 0) {
            q.subquestions.forEach((sub: any, subIdx: number) => {
                ...
                if (paperType === 'Structure') {
                    ...
                    doc.rect(margin + 20, currentY, 150, 20); // Answer box
                    currentY += 30;
                }
            });
        } else if (paperType === 'Structure') {
            // If no subquestions, provide an answer box
            if (currentY > pageHeight - 30) {
                doc.addPage();
                currentY = 20;
            }
